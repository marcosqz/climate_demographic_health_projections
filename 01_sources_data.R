...
model_param <- list(
  age_groups = c("00_74", "75plus"), # Group 1 = 0 to 74 years old
  # Group 2 = 75 and plus years old
  ssp_scenario = 2,
...
range(data_tempmort$year) # Study period

# RE-GROUP AGE CATEGORIES (00_74 AND 75_PLUS)
# TODO: I THINK THIS SHOULD BE BETTER ALREADY BE DONE IN THE SAVED DATA
...
# Keep only specific columns and change the name of the object
data_tempmort <- data_tempmort[, c("date", "year", "dow", "tmean", "mort.00_74", 
                                   "mort.75plus")]
data_tempmort$dow <- wday(data_tempmort$date)
...
data_popu_00_74 <- data_popu[
  data_popu$age %in% c("Y_LT1", paste0("Y", 1:74)), ]
# AGE GROUP 75 PLUS(75 to 99 and open-ended age class)
data_popu_75plus <- data_popu[
  data_popu$age %in% c(paste0("Y", 75:99), "Y_OPEN"), ]
...
proj_popu <- get_wcde(
  ...
  scenario = model_param$ssp_scenario,
  pop_age =  "all",
  pop_sex = "both",
...
# CALCULATE TOTAL MORTALITY AND POPULATION
...
# PLOT PROJECTIONS OF MORTALITY
par(mfrow = c(1, 2), oma = c(0, 0, 3, 0))
plot(x = proj_mortpopu$year, 
     y = proj_mortpopu$mort.00_74, type = "l",
     ylim = c(0, max(proj_mortpopu$mort.00_74, proj_mortpopu$mort.75plus)),
     main = "Mortality projections", 
     xlab = "Year", ylab = "Number of deahts", 
     col = "#1B9E77", lwd = 2)
lines(x = proj_mortpopu$year, 
      y = proj_mortpopu$mort.75plus, 
      lwd = 2, col = "#D95F02")
abline(h = 0)
legend("bottomleft", legend = c("<74", "+75"), fill = c("#1B9E77", "#D95F02"), 
       border = "black", bty = "o", cex = 0.8)

# PLOT PROJECTIONS OF POPULATION PERCERTANGE
plot(x = proj_mortpopu$year, 
     y = rep(0.5, length(proj_mortpopu$year)), 
     type = "n", ylim = c(0, 100),
     main = "Populations projections", 
...
legend("bottomright", legend = c("<74"), fill = c("#1B9E77"), 
       border = "black", bty = "o", cex = 0.8)
legend("topright", legend = c("+75"), fill = c("#D95F02"), 
       border = "black", bty = "o", cex = 0.8)

mtext(paste0("SSP", model_param$ssp_scenario, " - UK and Northern Ireland"), outer = TRUE, 
             cex = 1.5)
...
if(run_download == TRUE) {
  nex_download(location = getwd(),
               model = model_param$selected_gcms,
               scenario = model_param$ssp_rcp_scenario,
               variable = 'tas',
               years = 2015:2100,
               version = NULL,
...
proj_temp <- lapply(model_param$selected_gcms, function(i_gcm){ # LOOP SELECTED GCMS
...
# SOME GCMs MISS THE 31ST DECEMBER OF LEAP YEARS 
...
for(i_date in na_dates){
  ...
  proj_temp[proj_temp$date == i_date, "temp.BCC-CSM2-MR"] <- 
    mean(proj_temp[proj_temp$date %in% c(i_date-1, i_date+1), "temp.BCC-CSM2-MR"])
}
rm(i_date, na_dates)
...
# file_path <- paste0("../../tas/ssp245/", model_param$selected_gcms[1], 
#                     "/tas_day_", model_param$selected_gcms[1], 
#                     "_ssp245_r1i1p1f1_", var_path[model_param$selected_gcms[1]],
#                     "_", 2100, ".nc")
# raster_data <- raster::brick(file_path)
# 
# # Some NetCDF files have longitudes ranging from 0 to 360 instead of -180 to 180.
...
# }
# 
# # pdf(file = "../../outdata/plot/fig1_resolution_ssp_data.pdf", 
# #     width = 5, height = 5)
# plot(raster_data[[226]], xlab = "Longitude", ylab = "Latitude",
#      main = "Mean temperature 2100-08-15 \n(SSP2-RCP4.5, KIOST-ESM)")
...
data_warming <- lapply(model_param$selected_gcms, function(x) {
...
data_warming <- data_warming[, 
   grepl(model_param$ssp_rcp_scenario, colnames(data_warming)) | # columns that include the name of the ssp/rcp scenario
...
save(model_param, file = "../../outdata/file/mode_parameters.RData")
